...
class DoctorDashboardApp extends StatelessWidget {
  final String jwtToken; // ✅ Store token here
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: DoctorDashboardScreen(jwtToken: jwtToken), // ✅ Pass token to screen
    );
  }
}
...
class DoctorDashboardScreen extends StatefulWidget {
  ...
  const DoctorDashboardScreen({
    super.key,
    required this.jwtToken, // ✅ Mark it as required
  });
...
class _DoctorDashboardScreenState extends State<DoctorDashboardScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: _HeaderBar(onBellTap: () => _snack('Opening notifications')),
              ...
              child: Column(
                ...
                children: [
                  ...
                  QuickActionsGrid(
                    onTap: (label) {
                      if (label == 'Prescribe') {
                        // ✅ Navigate to PrescribePage with token
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder:
                                (context) =>
                                    PatientSelectApp(jwtToken: widget.jwtToken),
                          ),
                        );
                      } else {
...
class _HeaderBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  const _HeaderBar({required this.onBellTap});
  @override
...
class _AlertLine extends StatelessWidget {
  final String bold;
  final String rest;
  const _AlertLine({required this.bold, required this.rest});
